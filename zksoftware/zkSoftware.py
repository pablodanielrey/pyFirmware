...
class ZkSoftware:
    ...
    """
    <Restart>
        <ArgComKey>ComKey</ArgComKey>
    </Restart>
    """
    def restart(self):
        method = "<Restart><ArgComKey>{}</ArgComKey></Restart>"
        methodWithParams = method.format(0)
    ...
    """
    def refreshDb(self):
        method = "<RefreshDB><ArgComKey>{}</ArgComKey></RefreshDB>"
        methodWithParams = method.format(0)
        response = self._sendAndReceive(methodWithParams)
        return response
    ...
    """
    def setUserInfo(self, pin, passw, group, priv, card):
        method = "<SetUserInfo>" \
                    ...
                    "</Arg>" \
                "</SetUserInfo>"
        methodWithParams = method.format(pin, "", passw, group, priv, card, pin, 0, 0, 0)
        response = self._sendAndReceive(methodWithParams)
        return response['SetUserInfoResponse']['Row']
    ...
    """
        <SetUserTemplate>
            ...
            <Arg>
                ...
                <Size xsi:type="xsd:integer”>Template Size</Size>
                <Valid xsi:type="xsd:integer”>Valid 0|1</Valid>
                <Template xsi:type="xsd:integer”>Template Size</Template>
            </Arg>
        </SetUserTemplate>
    """
    def setUserTemplate(self, pin, fId, size, valid, template):
        method = "<SetUserTemplate>" \
                    "<ArgComKey>0</ArgComKey>" \
                    "<Arg>" \
                        "<PIN>{}</PIN>" \
                        "<FingerID>{}</FingerID>" \
                    ...
                    "</Arg>" \
                "</SetUserTemplate>"
        methodWithParams = method.format(pin, fId, size, valid, template)
        response = self._sendAndReceive(methodWithParams)
        return response['SetUserTemplateResponse']['Row']
